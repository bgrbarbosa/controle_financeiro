...
@Service
public class JasperServiceLancamentoReceita {
    ...
    public byte[] gerarPdfStatus() {
        byte[] bytes = null;
        try {
            Resource resource = resourceLoader.getResource(JASPER_DIRETORIO.concat("lancamento_receitas_status.jasper"));
    ...
    public byte[] gerarPdfPeriodo() {
        byte[] bytes = null;
        try {
            Resource resource = resourceLoader.getResource(JASPER_DIRETORIO.concat("lancamento_receitas_periodo.jasper"));
            InputStream stream = resource.getInputStream();
            JasperPrint print = JasperFillManager.fillReport(stream, params, dataSource.getConnection());
            bytes = JasperExportManager.exportReportToPdf(print);
        } catch (IOException | JRException | SQLException e) {
